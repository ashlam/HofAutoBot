...
def test_with_variations():
    ...
    print("=== 测试原始HTML ===\n")
    test_regex(original_content)
    
    # 测试变体1: 添加额外空格和换行
    print("\n\n=== 测试变体1: 添加额外空格和换行 ===\n")
    variant1 = original_content.replace('<div class="u">TOP 5</div>', '<div class="u">\n  TOP 5\n</div>')
    variant1 = variant1.replace('crown01.png', 'crown01.png  ')
    test_regex(variant1)
    
    # 测试变体2: 修改属性顺序
    print("\n\n=== 测试变体2: 修改属性顺序 ===\n")
    variant2 = original_content.replace('<td class="td7" valign="middle" style="text-align:center">', 
                                      '<td valign="middle" class="td7" style="text-align:center">')
    test_regex(variant2)
    
    # 测试变体3: 移除TOP 5标记
    print("\n\n=== 测试变体3: 移除TOP 5标记 ===\n")
    variant3 = original_content.replace('<div class="u">TOP 5</div>', '')
    test_regex(variant3)

def test_regex(content_text):
    """测试给定HTML内容的正则表达式匹配"""
    # 测试当前用户名匹配
