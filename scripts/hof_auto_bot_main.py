...
class HofAutoBot:
    ...
    def _process_boss_battle(self):
        ...
        if self.player_stamina < self.auto_bot_config_manager.boss_cost_stamina:
            ...
            print(f'体力不足，无法挑战boss，等待{recover_time}秒后重试')
            if recover_time > 0:
        ...
        if self.challenge_next_cooldown > 0:
            if self.challenge_next_cooldown >= self.IDLE_SECONDS_FOR_CHALLENGE_BOSS:
                ...
                print(f'BOSS冷却还早，当前体力：{self.player_stamina}，下次boss挑战时间：{next_challange_real_time}，于是去干别的')
                self._set_state(self.GAME_STATE_PVP)
                return
                
            # 耐心等一下就能打boss了
            else:
                print(f'Boss挑战冷却中，还剩{self.challenge_next_cooldown}秒，等待冷却结束')
        ...
        for vip_boss in self.auto_bot_config_manager.vip_boss_need_watch:
            ...
            if union_id in self.all_alived_boss_ids:
                ...
                if not next_battle_info:
                    ...
                    print(f'未找到上次vip boss被击败的时间，id: {union_id} ，没有办法处理boss({union_id})，去处理下一个vip boss。')
                    continue
                else:
                    # 计算下次刷新时间
                    ...
                    print(f"vip boss({union_id})将于{next_spawn_time}刷新，距离刷新还有{minutes}分{seconds}秒")
                    wait_time = max(0, time_until_spawn - self.COOLDOWN_SECONDS_FOR_CHALLENGE_BOSS * 2)
                    print(f"{ '将于%.2f秒后暂停' % wait_time if wait_time > 0 else '已暂停' }挑战普通boss行为，专心等vip")

                    # 如果很快就会刷新，等一等直接打
                    if time_until_spawn < self.COOLDOWN_SECONDS_FOR_CHALLENGE_BOSS * 2:
    ...
    def _process_wait_vip_boss(self):
        ...
        if self.waiting_vip_boss_time <= 0:
            ...
            print("在等vip boss，但时间太久了，干点别的去，省的被踢掉！")
            print(f"...但也不能刷太快发个呆。{next_active_time}后继续行动")
            ...
            if self.waiting_vip_boss_time > 0:
                idle_time = max(self.waiting_vip_boss_time - self.IDLE_SECONDS_FOR_REFRESH, self.IDLE_SECONDS_FOR_REFRESH)
                if idle_time > 0:
